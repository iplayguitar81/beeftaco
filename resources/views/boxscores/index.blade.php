...
<div class="col-md-12">
    ...
    <table class="table">
        ...
        @foreach($boxscores as $item)
            ...
            <tr>
                {{--<td>{{ $x }}</td>--}}
                <td><a href="{{ route('boxscores.show', [$item->id, str_slug($item->title)]) }}">{{ $item->title }}</a> <span class="pull-left"><br/>{{'Post images: '. $item->images()->count() }}</span></td><td>{{ $item->subHead }}</td><td>{{ $item->created_at->format('M dS Y') }}</td><td>
